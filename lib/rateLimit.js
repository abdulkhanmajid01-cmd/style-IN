...
// Local dev mein koi asli client IP nahi milta (proxy nahi hai), to pehle
// har request ka key "unknown" ban jata tha — yaani SAARI local traffic ek
// hi bucket mein. Iska matlab: developer 5 galat try karte hi 15 minute
// ke liye lock ho jata tha, aur phir har request 429 "Too many attempts"
// deti thi — jabki asli masla password ka nahi, apne aap ka tha.
//
// Isliye ab:
// 1) Local dev ke liye ek fixed bucket "local-dev" hai — sab kuch usi
//    ke andar jaata hai (grouped, jaisa chahiye tha), aur
// 2) production ke bahar blocking poori tarah band hai, taaki dev kabhi
//    apne aap ko lock na kare.
//
// SECURITY NOTE: yeh shortcut sirf `next dev` par chalta hai. Production
// build mein NODE_ENV="production" hota hai, wahan limiting poori tarah
// active rehti hai — koi kamzori production mein nahi jaati.
const LOCAL_BUCKET = "local-dev";
...
// Login route ise call karta hai har request ke shuru mein. `x-forwarded-for`
// me aksar ek hi request ke kai proxy hops ke IPs hote hain ("client, proxy1,
// proxy2") — asli client HAMESHA leftmost (pehla) entry hota hai, isliye
// poori list ko key banana galat hota (aur user aasaani se list badal kar
// apna lock bypass kar sakta hai). Isliye pehla entry hi lete hain.
...
export function isRateLimited(ip) {
  if (!isRateLimitingEnabled()) return false; // local dev: kabhi block nahi
...
// badhata hai, aur agar limit cross ho jaye to is IP ko block kar deta hai.
export function recordFailedAttempt(ip) {
  if (!isRateLimitingEnabled()) return; // local dev: count bhi nahi karte
